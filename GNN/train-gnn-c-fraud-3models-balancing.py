...
def train(model, data, optimizer, criterion, train_indices):
    """
    Função de treino modificada para aceitar os índices a serem usados.
    """
    model.train()
    optimizer.zero_grad()
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=500, help='Número de épocas.')
    ...
    if args.balancing == 'weights':
        ...
        print("Usando balanceamento por pesos de classe na função de perda.")
    else:
        criterion = torch.nn.NLLLoss()
        print("Balanceamento por pesos desativado.")
        
    # --- Preparar para Oversampling (se aplicável) ---
    ...
    if args.balancing == 'oversample':
        ...
        # Garantir que a amostragem seja possível
        if len(minority_indices) == 0:
             print("Aviso: Nenhuma amostra minoritária no conjunto de treino. Oversampling desativado.")
             args.balancing = 'none' # Reverter para 'none' se não for possível
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Determinar os índices de treino para a época atual
        if args.balancing == 'oversample':
            # Reamostrar a classe majoritária para igualar a minoritária
            sample_indices = np.random.choice(majority_indices.cpu().numpy(), len(minority_indices))
            # Combinar a classe minoritária completa com a amostra da majoritária
