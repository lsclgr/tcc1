...
class GAT(torch.nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, heads=8, dropout=0.6):
        ...
        # A primeira camada GAT usa múltiplas "cabeças de atenção"
        self.conv1 = GATConv(in_channels, hidden_channels, heads=heads, dropout=dropout)
        
        # A segunda camada GAT combina as cabeças e produz a saída final
...
def main():
    ...
    parser.add_argument('--model', type=str, default='sage', choices=['sage', 'gat'], help='Modelo a ser utilizado (sage ou gat).')
    ...
    result_dir = os.path.join("Results_GNN", f"Result_{timestamp}")
    ...
    if args.model.lower() == 'sage':
        model = GraphSAGE(in_channels=data.num_node_features, hidden_channels=args.hidden_channels, out_channels=2, num_layers=args.num_layers, dropout=args.dropout).to(device)
    elif args.model.lower() == 'gat':
        # GAT geralmente funciona bem com menos canais e mais cabeças de atenção
        gat_hidden = args.hidden_channels // 8 
        ...
        print(f"Modelo '{args.model}' não reconhecido. Use 'sage' ou 'gat'."); return
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 10 == 0:
            val_metrics, _, _ = evaluate(model, data, data.val_mask) # Avalia com limiar 0.5 por padrão
