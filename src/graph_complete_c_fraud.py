...
DATA_DIR = "data"
OUTPUT_DIR = "out"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "grafo_fraude_c.gml")
...
# CORREÇÃO AQUI: manter apenas level mais relevante
prioridade = {"Fraude": 2, "C": 1}
...
for _, row in levels.iterrows():
    ...
    if not matching_docs.empty:
        ...
        if documento in doc_features:
            atual = doc_features[documento]["level"]
            if level in prioridade:
                if atual is None or prioridade.get(level, 0) > prioridade.get(atual, 0):
                    doc_features[documento]["level"] = level
                    documentos_alvo.add(documento)

print("\nVerificação dos levels encontrados:")
levels_unicos = set(doc_features[d]["level"] for d in doc_features)
print(f"Levels presentes nos dados: {levels_unicos}")

print(f"\nDocumentos marcados como alvo (C/Fraude): {len(documentos_alvo)}")
for doc in list(documentos_alvo)[:5]:
    print(f"- {doc}: {doc_features[doc]['level']}")

if not documentos_alvo:
    raise ValueError("Nenhum documento com level 'C' ou 'Fraude' encontrado")
...
print("\n=== RESUMO FINAL ===")
print(f"Total de nós: {num_nos} (apenas documentos com level 'C' ou 'Fraude')")
