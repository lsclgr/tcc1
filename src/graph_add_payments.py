...
arquivos_csv = {
    ...
    "pagamento_conta": "data/pagamento_conta1.csv"
}
...
pagamento_conta = pd.read_csv(arquivos_csv["pagamento_conta"])
...
for _, row in documents.iterrows():
    ...
    if documento not in doc_features:
        doc_features[documento] = {
            ...
            "num_estornos": 0,  # Número de estornos
            "num_cancelamentos": 0,  # Número de cancelamentos
            "num_pagamentos_contas": 0,  # Número de pagamentos de contas
            "total_pagamentos_contas": 0.0  # Valor total de pagamentos de contas
        }
...
def add_transaction_edges(df, src_col, dst_col, trans_id_col, value_col, trans_type):
    for _, row in df.iterrows():
        ...
        if src_documento not in G:
            # Adiciona o nó com as informações existentes do dicionário doc_features
            G.add_node(src_documento, **doc_features.get(src_documento, {
                "document_id": "",
                "documento": src_documento,
                "documento_tipo": "",
                "account_info": [],
                "level": None,
        ...
        if dst_documento not in G:
            # Adiciona o nó com as informações existentes do dicionário doc_features
            G.add_node(dst_documento, **doc_features.get(dst_documento, {
                "document_id": "",
                "documento": dst_documento,
                "documento_tipo": "",
                "account_info": [],
                "level": None,
                "num_transacoes_feitas": 0,
        ...
        # Contar estornos e cancelamentos
        if "estorno" in row and row["estorno"] == 1:
            G.nodes[src_documento]["num_estornos"] += 1
        if "cancelamento" in row and row["cancelamento"] == 1:
            G.nodes[src_documento]["num_cancelamentos"] += 1
...
# Função para adicionar pagamentos de contas (sem destinatário específico)
def add_bill_payment_edges(df, src_col, trans_id_col, value_col):
    for _, row in df.iterrows():
        src_documento = row[src_col]
        trans_id, value = row[trans_id_col], row[value_col]
        trans_type = "pagamento_conta"  # Tipo fixo para esse caso

        # Verificar se o nó origem já existe no grafo
        if src_documento not in G:
            # Adiciona o nó com as informações existentes do dicionário doc_features
            G.add_node(src_documento, **doc_features.get(src_documento, {
                "document_id": "",
        ...
        # Atualizar métricas do nó (como transação feita)
        update_node_transaction_metrics(G.nodes[src_documento], value, trans_type, is_sender=True)
        
        # Incrementar contadores específicos de pagamentos de contas
        G.nodes[src_documento]["num_pagamentos_contas"] += 1
        G.nodes[src_documento]["total_pagamentos_contas"] += value
...
add_bill_payment_edges(pagamento_conta, "doc_origem", "transacao_id", "valor")
...
# Salvar grafo em GML
nx.write_gml(G, "out/grafo_payments1.gml")
...
print(f"Total de pagamentos de contas processados: {len(pagamento_conta)}")